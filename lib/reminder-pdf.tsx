...
const styles = StyleSheet.create({
  ...
  blockTitle: { fontSize: 12, fontWeight: 700, marginTop: 12, marginBottom: 6 },
...
function ReminderPdf({ data, logoUrl }: { data: ReminderPdfData; logoUrl: string }) {
  ...
  return (
    <Document title={`Herinnering ${invoice.invoice_number}`}>
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          <View style={styles.headerLeft}>
            <Image src={logoUrl} style={styles.logo} />
            <Text style={styles.title}>Betalingsherinnering</Text>
        ...
        <Text>
          Beste {customer.name},{"\n\n"}
          Volgens onze administratie is onderstaande factuur nog niet voldaan. Wilt u het openstaande bedrag binnen 7
          dagen overmaken onder vermelding van het factuurnummer?
        </Text>
...
export async function buildReminderPdfBuffer(data: ReminderPdfData, logoUrl: string): Promise<Uint8Array> {
  const instance = pdf(<ReminderPdf data={data} logoUrl={logoUrl} />);
